...
### 1.1 图的定义 (Definition of Graph)

- **图 (Graph)**: $G(V, E)$，其中：
  - $V = V(G)$: 顶点（vertex）的有限非空集合（finite nonempty set of vertices）
  - $E = E(G)$: 边（edge）的有限集合（finite set of edges）

- **无向图 (Undirected Graph)**: $(v_i, v_j) = (v_j, v_i)$，表示同一条边。
...
- **限制 (Restrictions)**:
  1. **自环 (Self loop)** 是不允许的（illegal）
  2. **多重图 (Multigraph)** 不考虑（not considered）

- **完全图 (Complete Graph)**: 具有最多边数的图（a graph that has the **maximum number of edges**）
...
|------|-----------|
| **邻接 (Adjacent)** | 无向图中：$v_i$ 和 $v_j$ 相邻（adjacent） |
| **关联 (Incident)** | $(v_i, v_j)$ 关联（incident）于 $v_i$ 和 $v_j$ |
| **邻接于/自 (Adjacent to/from)** | 有向图中：$v_i$ adjacent **to** $v_j$；$v_j$ adjacent **from** $v_i$；$\langle v_i, v_j \rangle$ 关联于 $v_i$ 和 $v_j$ |
| **子图 (Subgraph)** | $G' \subseteq G$ 满足 $V(G') \subseteq V(G)$ 且 $E(G') \subseteq E(G)$ |
| **路径 (Path)** | 从 $v_p$ 到 $v_q$ 的顶点序列 $\{v_p, v_{i1}, v_{i2}, \dots, v_{in}, v_q\}$，要求相邻顶点间存在边 |
| **路径长度 (Length of Path)** | 路径上**边的数量**（number of edges on the path） |
| **简单路径 (Simple Path)** | $v_{i1}, v_{i2}, \dots, v_{in}$ 都是不同的（distinct） |
| **环 (Cycle)** | 满足 $v_p = v_q$ 的简单路径（simple path with $v_p = v_q$） |

### 1.3 连通性 (Connectivity)

- **连通 (Connected)**（无向图）：$v_i$ 和 $v_j$ 之间存在一条路径。$v_i$ and $v_j$ in an undirected $G$ are **connected** if there is a path from $v_i$ to $v_j$ (and hence there is also a path from $v_j$ to $v_i$).
- **连通图 (Connected Graph)**：An undirected graph $G$ is **connected** if **every pair** of distinct $v_i$ and $v_j$ are connected.
- **连通分量 (Component / Connected Component)**：the **maximal** connected subgraph（极大连通子图）

### 1.4 特殊图 (Special Graphs)

| 类型 (Type) | 定义 (Definition) |
|------|-----------|
| **树 (Tree)** | a graph that is **connected** and **acyclic**（连通且无环） |
| **DAG** | a **Directed Acyclic Graph**（有向无环图） |
| **强连通 (Strongly Connected)**（有向图） | for **every pair** of $v_i$ and $v_j$ in $V(G)$, there exist **directed paths** from $v_i$ to $v_j$ **and** from $v_j$ to $v_i$ |
| **弱连通 (Weakly Connected)**（有向图） | If the graph is connected **without direction** to the edges, then it is said to be **weakly connected** |
| **强连通分量 (Strongly Connected Component / SCC)** | the **maximal** subgraph that is strongly connected |

### 1.5 度 (Degree)

- **度 (Degree) $degree(v)$**: number of edges incident to $v$（与 $v$ 关联的边的数量）
...
- **握手定理 (Handshaking Theorem)**: 给定有 $n$ 个顶点和 $e$ 条边的图 $G$，有：
...
- 若为**有向图**，还需要找出度/入度：
  - **方法 1**: 添加**逆邻接表**（inverse adjacency lists）
    - 示例：顶点 0->1, 1->0, 1->2
    - `inv[0]` -> 1
    - `inv[1]` -> 0
    - `inv[2]` -> 1
  - **方法 2**: 使用**多重表**（Multilist, Ch 3.2）表示 $adj\_mat[i][j]$
...
- 节点结构：`mark | v1 | v2 | next_v1 | next_v2`
  - **优势 (Advantage)**: 在检查一条边后需要标记（mark）它，然后找下一条边时，这个表示法很方便
...
### 2.1 AOV 网络 (Activity on Vertex Network)

- **AOV 网络**: 有向图 $G$，其中 $V(G)$ 表示**活动**（activities，如课程），$E(G)$ 表示**优先关系**（precedence relations）
  - 示例：$\langle C1, C3 \rangle$ 表示 $C1$ 是 $C3$ 的先修课程（prerequisite）

- **前驱 (Predecessor)**: 如果存在从 $i$ 到 $j$ 的路径，则 $i$ 是 $j$ 的**前驱**（predecessor）
- **直接前驱 (Immediate Predecessor)**: $\langle i, j \rangle \in E(G)$
  - 相应地，$j$ 是 $i$ 的**后继**（successor）或**直接后继**（immediate successor）

- **偏序关系 (Partial Order)**: 一种**传递性**（transitive）且**反自反**（irreflexive）的优先关系
  - **传递性 (Transitive)**: $i \to k, k \to j \Rightarrow i \to j$
  - **反自反性 (Irreflexive)**: $i \to i$ 是不可能的
...
**定义**：拓扑序（topological order）是图中顶点的一个线性排序，使得对于任意两个顶点 $i$, $j$，如果 $i$ 是 $j$ 的前驱，则 $i$ 在排序中出现在 $j$ 之前。
...
### 2.3 拓扑排序算法 (Topological Sort Algorithm)

**目标 (Goal)**: Test an AOV for feasibility, and generate a topological order if possible. （测试 AOV 的可行性，如果可能则生成一个拓扑序。）
...
**改进 (Improvement)**: Keep all the unassigned vertices of degree 0 in a special box (queue or stack).
...
| 顶点 (Vertex) | v1 | v2 | v3 | v4 | v5 | v6 | v7 |
|--------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
...
**问题描述 (Problem Description)**: Given a digraph $G = (V, E)$, and a cost function $c(e)$ for $e \in E(G)$. The length of a path $P$ from source to destination is $\sum c(e_i)$ (weighted path length).

给定一个有向图 $G = (V, E)$ 和边的代价函数 $c(e)$，路径 $P$ 从源点到目的地的长度是路径上所有边的代价之和。

---

### 3.1 单源最短路径问题 (Single-Source Shortest-Path Problem)

**输入 (Input)**: A weighted graph $G = (V, E)$ and a distinguished vertex $s$ (source).
**输出 (Output)**: Find the **shortest weighted path** from $s$ to **every other vertex** in $G$.

**负代价环 (Negative-cost cycle)**:
- 图中可能存在负代价环（negative-cost cycle）
- 如果存在负代价环，路径长度可以无限减小
- **注意 (Note)**: If there is **no negative-cost cycle**, the shortest path from $s$ to $s$ is defined to be **zero**.

---

### 3.2 无权最短路径 (Unweighted Shortest Paths)

#### 基本思路 (Breadth-First Search)

- 使用**广度优先搜索**（Breadth-first search / BFS）
...
> **性质 (Property)**: If $V$ is unknown yet has $Dist < Infinity$, then $Dist$ is either $CurrDist$ or $CurrDist + 1$.
...
| 出队 (Dequeue) | 邻接点 | Dist 更新 | 入队 (Enqueue) |
|:-------:|:------:|:---------:|:----:|
...
### 3.3 迪杰斯特拉算法 (Dijkstra's Algorithm) —— 加权最短路径
...
1. **性质 1 (Property 1)**: The shortest path must go through **ONLY** $v_i \in S$（最短路径必须只经过 $S$ 中的顶点）
   - **证明**: 如果该性质不成立，则路径上必然存在一个不在 $S$ 中的顶点 $w$，而 $w$ 的距离小于 $u$ 的距离，矛盾。

2. **性质 2 (Property 2)**: $u$ is chosen so that $distance[u] = \min\{ w \notin S \mid distance[w] \}$
   - 如果 $u$ 不唯一，可以任选一个

3. **性质 3 (Property 3)**: 如果 $distance[u_1] < distance[u_2]$ 并且我们将 $u_1$ 加入 $S$，则 $distance[u_2]$ 可能改变。新的更短路径可能经过 $u_1$:
...
| 步骤 (Step) | V (min unknown Dist) | Known | 更新邻接点 |
|:----:|:--------------------:|:-----:|:----------:|
...
#### 时间复杂度分析 (Complexity Analysis)

**实现 1 (Implementation 1)**: 扫描整个表找最小未知距离顶点
- 找最小未知距离: $O(|V|)$
- 总复杂度: $T = O(|V|^2 + |E|)$
- 适合**稠密图**（dense graph）

**实现 2 (Implementation 2)**: 使用优先队列（Priority Queue）
- 找最小未知距离: DeleteMin → $O(\log|V|)$
- 更新距离:
  - **方法 1 (Method 1)**: DecreaseKey → $O(\log|V|)$
    - $T = O(|V|\log|V| + |E|\log|V|) = O(|E|\log|V|)$
  - **方法 2 (Method 2)**: 将更新后的 $W$ 直接插入优先队列
...
> **重要限制 (Important Limitation)**: Dijkstra 算法**不能处理负权边**
> ```c
> /* Does NOT work for edges with negative costs */
> ```
...
### 3.4 含负权边的图 (Graphs with Negative Edge Costs)
...
#### AOE 网络 (Activity On Edge Networks)
...
| 术语 (Term) | 定义 (Definition) |
|------|-----------|
| **EC[j]** | **最早完成时间 (Earliest completion time)** for node $v_j$ |
| **LC[j]** | **最迟完成时间 (Latest completion time)** for node $v_j$ |
| **松弛时间 (Slack Time)** | 松弛时间 = LC - EC |
| **关键路径 (Critical Path)** | Path consisting entirely of **zero-slack** edges（由松弛时间为 0 的边组成的路径） |
...
**松弛时间 (Slack Time)** of $\langle v, w \rangle$：
...
**约束 (Constraint)**: Total coming in ($v$) $\equiv$ Total going out ($v$), where $v \notin \{s, t\}$
（对于除源点和汇点外的每个顶点，流入量等于流出量）

**目标 (Goal)**: Determine the **maximum amount of flow** that can pass from $s$ to $t$.（确定从源点到汇点能通过的最大流量）

### 4.2 简单算法 (A Simple Algorithm)

**基本步骤 (Basic Steps)**:

1. **Step 1**: Find any path $s \to t$ in $G_r$ (residual graph) — 找一条**增广路径** (augmenting path)
2. **Step 2**: Take the **minimum edge** on this path as the amount of flow and add to $G_f$ (flow graph) — 取路径上的最小边容量作为流量，加入**流量图** (flow graph)
3. **Step 3**: Update $G_r$ and remove the 0 flow edges — 更新**残余图** (residual graph)，移除流量为 0 的边
4. **Step 4**: If (there is a path $s \to t$ in $G_r$) Goto Step 1; Else End.

**残余图 $G_r$ 的更新**:
- 对于每条边 $(v, w)$，其**残余容量** (residual capacity) = 原容量 - 当前流量
- 当一条边的流量等于其容量时（饱和），从残余图中移除

#### 贪心的问题 (Problems with Greedy)
...
### 4.3 解决方案 —— 允许算法撤销其决定 (Allow the Algorithm to Undo Its Decisions)
...
如果边的容量是**有理数** (rational numbers)，该算法总是以最大流终止。

**Note**: The algorithm works for $G$ with cycles as well.（该算法也适用于有环图）

### 4.4 算法分析 (Algorithm Analysis)

**假设 (Assumption)**: capacities are all **integers**（容量均为整数）

**方法 1 (Method 1)**: 使用无权最短路径找增广路径
- An augmenting path can be found by an **unweighted shortest path algorithm** (BFS)
- 每次增广至少增加 1 单位流量
- 复杂度: $T = O(f \cdot |E|)$，其中 $f$ 是最大流量

**最坏情况示例 (Worst-case Example)**:
```
...
### 4.5 最小费用流问题 (Min-Cost Flow Problem)
...
在所有最大流中，找到总费用最小的流。每条边有一个单位流量费用（cost per unit of flow）。
...
一个图 $G$ 的**生成树 (spanning tree)** 是由 $V(G)$ 和 $E(G)$ 的一个子集组成的树。

**示例**: 一个完全图及其三个生成树（每个生成树都包含所有顶点，但边数 = $|V|-1$）

### 5.2 基本性质 (Basic Properties)

1. **树 (Tree)**: The minimum spanning tree is a **tree** since it is **acyclic** — the number of edges is $|V| - 1$.
2. **最小 (Minimum)**: It is minimum for the **total cost of edges** is minimized.
3. **生成 (Spanning)**: It is spanning because it **covers every vertex**.
4. **存在性 (Existence)**: A minimum spanning tree exists **iff** $G$ is connected.
5. **环性质 (Cycle Property)**: Adding a **non-tree edge** to a spanning tree, we obtain a **cycle**.
...
### 5.5 克鲁斯卡尔算法 (Kruskal's Algorithm)
...
| 算法 (Algorithm) | 策略 (Strategy) | 数据结构 (Data Structure) | 时间复杂度 (Time Complexity) |
|-----------|----------|----------------|-----------------|
...
- 这是**前序遍历** (preorder traversal) 的推广
- 使用 `visited` 数组标记已访问顶点，避免环
- 如果使用邻接表，时间复杂度 $T = O(|E| + |V|)$

### 6.2 无向图 —— 列出连通分量 (ListComponents)
...
**示例**:
给定图的结构：
- 0-1-4-6-5-2-3（连通分量 1 / connected component 1）
- 7-8（连通分量 2 / connected component 2）
...
### 6.3 双连通性 (Biconnectivity)

#### 基本概念 (Basic Concepts)

- **关节点 (Articulation Point / Cut Vertex)**: $v$ is an **articulation point** if $G' = DeleteVertex(G, v)$ has **at least 2 connected components**.
  - 删除顶点 $v$ 后，图分裂成至少两个连通分量。

- **双连通图 (Biconnected Graph)**: $G$ is **biconnected** if $G$ is connected and has **no articulation points**.

- **双连通分量 (Biconnected Component)**: a **maximal** biconnected subgraph.
  - **注意 (Note)**: No edges can be **shared** by two or more biconnected components. Hence $E(G)$ is **partitioned** by the biconnected components of $G$.
...
**关节点 (Articulation Points)**: 1, 7（删除后图会分裂）

#### 寻找双连通分量的算法 (Finding Biconnected Components)

**Step 1**: 使用 DFS 获得图的**深度优先生成树**（Depth First Spanning Tree）
...
#### 寻找关节点 (Finding Articulation Points)
...
| 顶点 (Vertex) | Num | Low | 子节点 (Child) | Low(child) >= Num(u)? | 关节点? |
|:------:|:---:|:---:|:------:|:---------------------:|:-------:|
...
> **参考 (Reference)**: 详细伪代码见 p.327 和 p.329。

---

### 6.4 欧拉回路 (Euler Circuits)

#### 欧拉路径与欧拉回路 (Eulerian Path and Eulerian Circuit)

**欧拉路径 (Euler Tour / Eulerian Path)**: Draw each line **exactly once** without lifting your pen from the paper.
（一笔画问题：每条边恰好画一次，笔不离开纸面）

**欧拉回路 (Euler Circuit / Eulerian Cycle)**: Draw each line exactly once without lifting your pen from the paper, **AND finish at the starting point**.
...
在无向图中找一个经过**每个顶点**恰好一次的简单环——**哈密顿回路 (Hamilton cycle)**。

> **注意**: 哈密顿回路与欧拉回路的区别：
> - **欧拉回路 (Euler Circuit)**: 经过**每条边**恰好一次
> - **哈密顿回路 (Hamilton Cycle)**: 经过**每个顶点**恰好一次

---

### 6.5 补充学习资源 (Supplementary Resources)
...
## 附录：各算法复杂度总结 (Summary of Complexities)

| 算法 (Algorithm) | 时间复杂度 (Time Complexity) | 空间复杂度 (Space) | 备注 (Notes) |
|-----------|----------------|-------|-------|
| **拓扑排序 (Topological Sort)** (queue) | $O(\|V\| + \|E\|)$ | $O(\|V\|)$ | DAG only |
| **无权最短路径 (Unweighted Shortest Path)** (queue) | $O(\|V\| + \|E\|)$ | $O(\|V\|)$ | BFS |
...
| **无环最短路径 (Acyclic Shortest Path)** | $O(\|V\| + \|E\|)$ | $O(\|V\|)$ | Topological order |
| **所有点对 (All-Pairs)** (Dijkstra x V) | $O(\|V\|^3)$ | $O(\|V\|^2)$ | Sparse graph |
| **所有点对 (All-Pairs)** (Floyd) | $O(\|V\|^3)$ | $O(\|V\|^2)$ | Dense graph |
| **网络流 (Network Flow)** (basic) | $O(f \cdot \|E\|)$ | $O(\|E\|)$ | $f$ = max flow |
| **DFS** | $O(\|V\| + \|E\|)$ | $O(\|V\|)$ | Adjacency lists |
| **Prim** | $O(\|E\| \log\|V\|)$ | $O(\|V\|)$ | Similar to Dijkstra |
| **Kruskal** | $O(\|E\| \log\|E\|)$ | $O(\|V\| + \|E\|)$ | Union/Find |
| **欧拉回路 (Euler Circuit)** | $O(\|V\| + \|E\|)$ | $O(\|V\| + \|E\|)$ | DFS-based |
| **双连通性 (Biconnectivity)** | $O(\|V\| + \|E\|)$ | $O(\|V\|)$ | DFS + Low values |

---

## 附录：关键公式汇总 (Key Formulas)

1. **握手定理 (Handshaking Theorem)**: $\sum_{v \in V} degree(v) = 2|E|$

2. **完全图的边数 (Complete Graph Edges)**:
   - 无向 (Undirected): $|E| = n(n-1)/2$
   - 有向 (Directed): $|E| = n(n-1)$

3. **对称矩阵存储 (Symmetric Matrix Storage)**: $index(a_{ij}) = i(i-1)/2 + j$ (for $i \geq j$)

4. **生成树的边数 (Spanning Tree Edges)**: $|E_{MST}| = |V| - 1$

5. **Dijkstra 更新 (Dijkstra Update)**: $distance'[u_2] = distance[u_1] + weight(\langle u_1, u_2 \rangle)$

6. **AOE 最早完成时间 (EC)**: $EC[w] = \max\{EC[v] + cost(v, w)\}$

7. **AOE 最迟完成时间 (LC)**: $LC[v] = \min\{LC[w] - cost(v, w)\}$

8. **松弛时间 (Slack Time)**: $Slack(v, w) = LC[w] - EC[v] - cost(v, w)$

9. **关节点判定条件 (Articulation Point Condition)**: $Low(child) \geq Num(u)$

10. **Low 值定义 (Low Value)**: $Low(v) = \min\{Num(v), \min Low(child), \min Num(back\:edge\:target)\}$
