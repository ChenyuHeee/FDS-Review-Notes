...
图示（幻灯片 3）展示了一个包含 8 个元素的数组：`4, -3, 5, -2, -1, 2, 6, -2`。

- **分步骤：** 分成 `[4, -3, 5, -2]` 和 `[-1, 2, 6, -2]`
- **进一步划分：** 左半部分分成 `[4, -3]` 和 `[5, -2]`；右半部分分成 `[-1, 2]` 和 `[6, -2]`
- **基本情况 (Base Case)：** `4`, `-3`, `5`, `-2`, `-1`, `2`, `6`, `-2`
- **向上合并：** 递归计算每个部分的最大值
  - `[4, -3]` 最大值为 4
  - `[5, -2]` 最大值为 5
  - 左半部分跨中点最大值为 `max(5+(-2)+(-3)=0, 4+(-3)=1, 5+(-2)=3...)`，跨中点最大和为 `4 + (-3) + 5 = 6`
  - 左半部分整体最大值为 6
  - `[-1, 2]` 最大值为 2
  - `[6, -2]` 最大值为 6
  - 右半部分跨中点最大值为 2 + 6 = 8
  ...
  - 跨整体中点的最大和 = 4 + 7 = 11
  - 整体最大值 = max(6, 8, 11) = 11
- 整体最大子序列和为 11（从 `A[0]` 到 `A[6]`：4 - 3 + 5 - 2 - 1 + 2 + 6 = 11）
