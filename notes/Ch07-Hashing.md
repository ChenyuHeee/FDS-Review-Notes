...
> **Search by Formula** — 不通过比较，而是通过一个函数直接计算出存储位置。

---

## Overview / 引言

有一个定理说："任何**仅通过比较**进行排序的算法，其最坏情况时间复杂度为 $\Omega(n \log_2 n)$。"
...
在有序列表 $f[l].key, f[l+1].key, \dots, f[u].key$ 中查找 key：
...
**定义**：符号表是一组 `< name, attribute >` 对，其中名字是唯一的。

**示例**：
- **牛津英语词典**：`name = "since"`, `attribute = 含义列表`
  - M[0] = after a date, event, etc.
  - M[1] = seeing that (expressing reason)
  - ......
- **编译器符号表**：`name = identifier (e.g., int)`, `attribute = 使用该标识符的行列表及其他信息`

### 符号表 ADT

**对象**：一组 name-attribute 对，names 唯一。
...
对于每个标识符 $x$，定义一个**哈希函数** $f(x)$ = $x$ 在哈希数组 `ht[]` 中的位置（即包含 $x$ 的桶的索引）。
...
| 术语 | 定义 |
|------|------|
| **Collision (冲突)** | 当两个不同的标识符 $i_1 \neq i_2$ 被哈希到同一个桶时，即 $f(i_1) = f(i_2)$ |
| **Overflow (溢出)** | 当一个新标识符被哈希到一个已满的桶时 |

> 当 $s = 1$（每个桶只有 1 个槽位）时，Collision 和 Overflow 同时发生。

**示例**：将 $n = 10$ 个 C 库函数映射到哈希表 `ht[]`，$b = 26$ 个桶，$s = 2$ 个槽位。

加载密度 $\lambda = 10 / (2 \times 26) = 10 / 52 = 0.19$

哈希函数定义：$f(x) = x[0] - \text{'a'}$（将字母 a~z 映射到 0~25）。
...
> **关键结论**：如果没有溢出（overflow），则**查找、插入、删除的时间复杂度均为 $O(1)$**。
...
- **问题**：如果 TableSize = 10，且所有 $x$ 都以 0 结尾（如 10, 20, 30...），则所有键都映射到同一位置。
- **解决方案**：**TableSize 应选为质数** — 对随机整数键效果良好。

#### 2. 字符串键的哈希 — 简单求和

$$f(x) = \left( \sum x[i] \right) \;\%\; \text{TableSize}$$

**示例**：TableSize = 10,007，字符串长度 $\leq 8$，$x[i] \in [0, 127]$。
...
**说明**：此代码与通用 List ADT 的 Find 操作完全相同。
...
**注意**：插入操作采用**头插法**（在链表头部插入新节点）。

### 性能提示

> **Tip**: 让 TableSize 约等于期望的键的数量（即保持负载密度 $\lambda \approx 1$）。

---

## Section 4: Open Addressing (开放地址法)

**核心思想**：不利用指针，当冲突发生时寻找**另一个空单元**来解决。
...
**示例**：将 $n = 11$ 个 C 库函数映射到哈希表，$b = 26$ 个桶，$s = 1$。
...
**定义**：任何哈希到集群中的键，都需要多次探测才能解决冲突，从而**使集群变得更大**。
...
```
随着插入变多，连续被占用的桶形成"集群"，即使 $\lambda$ 很小，最坏情况也可能变得**非常大**。
...
> **【Theorem】** 如果使用二次探测，**且表大小为质数**，则当**表至少一半为空**时，总能插入一个新元素。
...
因为 TableSize 是质数，所以 $(i + j)$ 或 $(i - j)$ 必有一个能被 TableSize 整除。
...
**Q**: 如何删除一个键？
**A**: 不能直接删除，需要使用**惰性删除**（标记为 Deleted 而非真正删除）。

**注意**：
1. 如果插入和删除操作频繁交替，Insert 操作会**严重变慢**（因为标记为 Deleted 的单元被视为已被占用，导致探测链中断）。
2. 虽然解决了 **Primary Clustering**，但**Secondary Clustering (二次聚集)** 仍然存在——即哈希到同一位置的键会探测相同的备选单元序列。
...
1. **建立一个大约是原表两倍大的新表**
2. **扫描整个原哈希表**，找到所有未被删除的元素
3. **使用新的哈希函数**将这些元素重新哈希到新表中
...
> **Note**: 通常在进行 Rehash 之前已经有 $N/2$ 次插入操作，因此 $O(N)$ 的 Rehash 时间**平摊**到每次插入上只是**常数级额外开销**。

**但需注意**：在交互式系统中，触发 Rehash 的那次插入操作的用户会**感受到明显的延迟**。
...
| 策略 | 方法 | 优点 | 缺点 |
|------|------|------|------|
| **Separate Chaining** | 链表存储同桶元素 | 实现简单，$\lambda$ 可 > 1 | 需要指针，额外空间 |
...
### 关键要点
1. **TableSize 应选质数** — 对哈希分布有重要意义
2. **负载密度 $\lambda$** 直接影响性能：
   - Separate Chaining: $\lambda \approx 1$ 推荐
   - Open Addressing: 通常 $\lambda < 0.5$
3. **二次探测**是实践中常用的折衷方案
4. **Rehashing** 解决表满问题，平摊成本为 $O(1)$ 每次插入
5. 理想的哈希函数应当是**均匀的 (uniform)** 且**易于计算**
